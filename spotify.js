...
songs.forEach(element => {
    ...
    card.innerHTML = `
        <img src="${poster}" alt="">
        <div class="content">${songName}</div>`; // Remove the semicolon after songName
...
// Iterate over each search result item and add a click event listener
Array.from(search_result.getElementsByTagName('a')).forEach(item => {
    item.addEventListener('click', () => {
        // Extract the ID of the clicked song
        const songId = parseInt(item.href.split('#')[1]);
        ...
        if (selectedSong) {
            ...
            // Update the player UI with the selected song information
            poster_master_play.src = selectedSong.poster;
            title.innerHTML = selectedSong.songName;
            download_music.href = `audio/${selectedSong.id}.mp3`;
            
            // Highlight the selected song in the search results
            Array.from(search_result.getElementsByTagName('a')).forEach(item => {
                item.style.background = 'rgb(105, 105, 105, .0)';
            });
